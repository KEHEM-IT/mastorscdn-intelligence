...
const scssCompletionProvider_1 = require("./providers/scssCompletionProvider");
...
async function activate(context) {
    ...
    const completionProvider = new scssCompletionProvider_1.ScssCompletionProvider(registry);
    const completionDisposable = vscode.languages.registerCompletionItemProvider(LANG_SELECTORS, completionProvider, '.', // mc.  → show function list
    '(', // mc.color(  → show values
    "'", // mc.color('  → show values
    '"' // mc.color("  → show values
    );
    // ── 3. Register hover provider ────────────────────────────────────────────
    const hoverProvider = new hoverProvider_1.HoverProvider(registry);
    const hoverDisposable = vscode.languages.registerHoverProvider(LANG_SELECTORS, hoverProvider);
    // ── 4. Register signature help provider ──────────────────────────────────
    const signatureProvider = new signatureHelpProvider_1.SignatureHelpProvider(registry);
    const signatureDisposable = vscode.languages.registerSignatureHelpProvider(LANG_SELECTORS, signatureProvider, '(', // retrigger on open paren
    ',' // retrigger on comma
    );
    // ── 5. Commands ───────────────────────────────────────────────────────────
    ...
    const generateRegistryCommand = vscode.commands.registerCommand('mastorsIntellisense.generateRegistry', async () => {
        vscode.window.withProgress({
            ...
            if (path) {
                vscode.window.showInformationMessage(`✅ Registry JSON written to: ${path}`, 'Open File').then((action) => {
                    if (action === 'Open File') {
...
function showEntryWebview(context, entry) {
    const panel = vscode.window.createWebviewPanel('mastorsEntry', `Mastors: ${entry.name}`, vscode.ViewColumn.Beside, { enableScripts: false });
    const paramsHtml = entry.params
        ...
        .join('');
    const valuesHtml = entry.values && entry.values.length > 0
